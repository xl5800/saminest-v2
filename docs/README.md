...
1. 降低 AI 修改无关代码的风险。
2. 避免一次性大规模重写。
3. 防止重复创建运行入口、Supabase Client 和全局监听器。
4. 保持代码、数据库和文档一致。
5. 保证每次修改可验证、可审查、可回滚。
...
Saminest 当前处于从旧版 JavaScript 向 TypeScript 模块化架构迁移的阶段。
...
- `index.html` 加载哪些脚本
- `src/main.ts` 是否接管功能
- `public/legacy-app.js` 是否仍在运行
- 新旧实现是否同时存在
- 是否有重复事件监听
- 是否有重复 Supabase Client

不得仅修改没有被实际加载的文件，然后声称功能已经修复。
...
AI 必须遵守 `Architecture.md` 中定义的架构方向。

### 5.1 单一 Supabase Client

全项目浏览器运行时只能存在一个 Supabase Client 初始化入口。
...
散落在页面、Feature、Service 或 Repository 中。
...
`onAuthStateChange` 只能由统一认证模块注册一次。
...
除非任务是修复当前仍由 legacy 执行的紧急 Bug，否则新功能不得继续大量加入：
...
container.addEventListener('click', (event) => {
  ...
  const button = target.closest<HTMLButtonElement>('[data-action="favorite"]');
...
复杂行为应由 TypeScript 模块管理。

---

### 7.4 用户内容必须安全输出

不得直接把未经处理的用户输入拼入 HTML。
...
Auth listener 数量
Supabase ready listener 数量
Supabase error listener 数量
legacy-app.js 加载数量
app.js 加载数量
```
...
npm run check:legacy：通过 / 失败 / 未运行
...
特别是删除 `legacy-app.js` 中的逻辑时，必须确认 TypeScript 实现已经实际运行。
...
```ts
// TODO(tech-debt): Remove this compatibility adapter after the
// publish module is fully migrated from legacy-app.js.
```
...
6. 不扩大 legacy-app.js，除非当前功能仍只能在 legacy 中修复。
...
目标：
[例如：从 legacy-app.js 提取到 TypeScript]
...
基于 Saminest 当前迁移阶段，必须长期保持以下基线：

```text
createClient() 入口：1
Supabase Client 实例：1
Auth listener：1
Supabase ready listener：1
Supabase error listener：1
legacy-app.js 加载：1
旧 app.js 加载：0
```
...
当前新代码优先写入 TypeScript。

`public/legacy-app.js` 只允许：
...
```text
docs: update AI verification requirements

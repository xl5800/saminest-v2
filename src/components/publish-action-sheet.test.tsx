...
describe("PublishActionSheet", () => {
  ...
  it("renders the four publish options in the default order on a non-activities page", () => {
  ...
  it("puts '发起搭子' first and visually emphasizes it when opened from an /activities page", () => {
    navigateMock.mockReset();
    renderWithProviders(<PublishActionSheet onClose={vi.fn()} />, {
    ...
    const buttons = screen.getAllByRole("button").filter((button) => button.textContent !== "取消");
    expect(buttons[0].textContent).toBe("👥发起搭子");
    expect(buttons[0].className).toContain("font-semibold");
  });

  it("also prioritizes '发起搭子' on a nested activities path (e.g. an activity detail page)", () => {
    navigateMock.mockReset();
    renderWithProviders(<PublishActionSheet onClose={vi.fn()} />, {
  ...
  it("does not emphasize '发起搭子' on a non-activities page", () => {
    navigateMock.mockReset();
    renderWithProviders(<PublishActionSheet onClose={vi.fn()} />, {
      initialEntries: ["/"]
    });
    ...
    expect(activityButton.className).not.toContain("font-semibold");
  });

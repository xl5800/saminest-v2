...
describe("MyPostsPage", () => {
  ...
  // 真实列表项形状（缩略图+标题+徽章+次要信息）的骨架块，sr-only 播报
  // 文字保留、骨架块本身是纯视觉装饰。
  it("renders skeleton placeholder list items, not a plain 加载中 paragraph, while pending", () => {
    listMyPosts.mockReturnValue(new Promise(() => {}));

    const { container } = renderWithProviders(<MyPostsPage />);

...
import { ReportActivityPage } from "./report-activity-page";

const initialAuthState = useAuthStore.getState();

function renderPage() {
  return renderWithProviders(<ReportActivityPage />, {
    initialEntries: ["/activities/act-1/report"],
    route: "/activities/:id/report"
  });
}
...
describe("ReportActivityPage", () => {
  ...
  it("renders the '举报活动' heading and the reason options when logged in", () => {
  ...
  it("submits the report with targetType 'activity' and the activity id from the route, and shows the success feedback", async () => {
    useAuthStore.getState().setSession({ user: { id: "user-1" } } as never);

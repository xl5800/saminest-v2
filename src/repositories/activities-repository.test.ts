...
describe("listActivityParticipants", () => {
  ...
  it("queries non-cancelled, approved participants for the activity, ordered by joined_at ascending, with a nested profile select including age/bio/locationName", async () => {
    overrideTypesMock.mockResolvedValue({ data: [], error: null });
    ...
    // 任务卡（"已加入"名单加头像/简介）：同样理由再加一列 bio。
    expect(queryBuilder.select).toHaveBeenCalledWith(
      "user_id, user:profiles(display_name, avatar_url, age, bio, location:locations(name))"
    );
    expect(queryBuilder.eq).toHaveBeenCalledWith("activity_id", "act-1");
  ...
  it("maps rows to ActivityParticipant, including avatarUrl/age/bio/locationName", async () => {
    overrideTypesMock.mockResolvedValue({
      data: [
        {
          user_id: "user-1",
        ...
        {
          user_id: "user-2",
          user: { display_name: "Bob", avatar_url: null, age: null, bio: null, location: null }
        }
      ],
      error: null
    });
    ...
    expect(result).toEqual([
      ...
      { userId: "user-2", displayName: "Bob", avatarUrl: null, age: null, bio: null, locationName: null }
    ]);
  });

  it("falls back to a placeholder display name, and null avatarUrl/age/bio/locationName, when the joined profile is missing", async () => {
    overrideTypesMock.mockResolvedValue({
...
describe("listActivityParticipantPreviews", () => {
  ...
  it("queries approved, non-cancelled participants across the given activity ids, ordered by joined_at ascending, with a nested profile select including age/bio/locationName", async () => {
    overrideTypesMock.mockResolvedValue({ data: [], error: null });
    ...
    // select 语句也带上了 age/bio/location:locations(name)——即使活动卡片/
    // ActivityParticipantAvatars（这个函数唯一的消费方）不读这些字段，
    // 见 activities-repository.ts 里 mapActivityParticipantRow 的注释。
    expect(queryBuilder.select).toHaveBeenCalledWith(
      "activity_id, user_id, user:profiles(display_name, avatar_url, age, bio, location:locations(name))"
    );
  ...
  it("groups rows by activity_id into a Map<activityId, ActivityParticipant[]>, including age/bio/locationName", async () => {
    overrideTypesMock.mockResolvedValue({
      data: [
        {
          activity_id: "act-1",
        ...
        },
        {
          activity_id: "act-2",
          user_id: "user-3",
          user: { display_name: "Carol", avatar_url: null, age: null, bio: null, location: null }
        }
      ],
      error: null
    });
    ...
    expect(result.get("act-2")).toEqual([
      { userId: "user-3", displayName: "Carol", avatarUrl: null, age: null, bio: null, locationName: null }
    ]);
  ...
  it("falls back to a placeholder display name, and null avatarUrl/age/bio/locationName, when the joined profile is missing", async () => {
    ...
    expect(result.get("act-1")).toEqual([
      { userId: "user-1", displayName: "未知用户", avatarUrl: null, age: null, bio: null, locationName: null }
    ]);
  });
